 ...
 * @author Jim Chen
 */
window.BinArray = (function(){
	window.BinArray = {};
	window.BinArray.bsearch = function(arr, what, how){
		if(arr.length === 0) {
			return 0;
		}
		if(how(what,arr[0]) < 0) {
	...
	window.BinArray.binsert = function(arr, what, how){
		var index = window.BinArray.bsearch(arr,what,how);
		arr.splice(index,0,what);
		return index;
	};
	return window.BinArray;
})();
...
var CommentSpaceAllocator = (function () {
    ...
    CommentSpaceAllocator.prototype.add = function (comment) {
        if (comment.height > this._height) {
            ...
            window.BinArray.binsert(this._pools[comment.cindex], comment, function (a, b) {
...
window.CommentManager = (function() {
	...
	CommentManager.prototype.seek = function(time){
		this.position = window.BinArray.bsearch(this.timeline, time, function(a,b){
	...
	CommentManager.prototype.insert = function(c){
		var index = window.BinArray.binsert(this.timeline, c, function(a,b){
			if(a.stime > b.stime) return 2;
			else if(a.stime < b.stime) return -2;
			else{
				if(a.date > b.date) return 1;
...
function BilibiliParser(xmlDoc, text, warn){	
	...
	if(xmlDoc !== null){
		...
		if(warn){
			if(!window.confirm("XML Parse Error. \n Allow tag soup parsing?\n[WARNING: This is unsafe.]")){
				return [];
			}
		}else{
	...
	for(var i=0;i < elems.length;i++){
		if(elems[i].getAttribute('p') != null){
			...
			if(obj.mode < 7){
				...
				if(obj.mode == 7){
					try{
						window.adv = JSON.parse(format(text));
						obj.shadow = true;
						obj.x = parseFloat(window.adv[0]);
						obj.y = parseFloat(window.adv[1]);
						if(Math.floor(obj.x) < obj.x || Math.floor(obj.y) < obj.y){
							obj.position = "relative";
						}
						obj.text = window.adv[4].replace(/(\/n|\\n|\n|\r\n)/g, "\n");
						obj.rZ = 0;
						obj.rY = 0;
						if(window.adv.length >= 7){
							obj.rZ = parseInt(window.adv[5], 10);
							obj.rY = parseInt(window.adv[6], 10);
						}
						obj.motion = [];
						obj.movable = false;
						if(window.adv.length >= 11){
							obj.movable = true;
							var singleStepDur = 500;
							var motion = {
								x:{from: obj.x, to:parseFloat(window.adv[7]), dur:singleStepDur, delay:0},
								y:{from: obj.y, to:parseFloat(window.adv[8]), dur:singleStepDur, delay:0},
							};
							if(window.adv[9] !== ''){
								singleStepDur = parseInt(window.adv[9], 10);
								motion.x.dur = singleStepDur;
								motion.y.dur = singleStepDur;
							}
							if(window.adv[10] !== ''){
								motion.x.delay = parseInt(window.adv[10], 10);
								motion.y.delay = parseInt(window.adv[10], 10);
							}
							if(window.adv.length > 11){
								obj.shadow = window.adv[11];
								if(obj.shadow === "true"){
									obj.shadow = true;
								}
								if(obj.shadow === "false"){
								...
								if(window.adv[12] != null){
									obj.font = window.adv[12];
								}
								if(window.adv.length > 14){
									// Support for Bilibili window.advanced Paths
									if(obj.position === "relative"){
										console.log("Cannot mix relative and absolute positioning");
										obj.position = "absolute";
									}
									var path = window.adv[14];
						...
						obj.dur = 2500;
						if(window.adv[3] < 12){
							obj.dur = window.adv[3] * 1000;
						}
						var tmp = window.adv[2].split('-');
